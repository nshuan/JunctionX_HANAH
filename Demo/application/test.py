...
def test(test_dir, output, down_scale, frame_count):
    ...
    for scene, folder in enumerate(os.listdir(video_path)):
        ...
        for camera in os.listdir(os.sep.join([video_path, folder])):
            ...
            if not os.path.exists(os.sep.join([output, f'{folder}',camera[:-3]])):
               os.mkdir(os.sep.join([output, f'{folder}',camera[:-3]]))
        k = 0
        while True and k < frame_count:
            ...
            for i in range(cam):
                cv2.imwrite(os.sep.join([output, f'{folder}',f'CAM_{i+1}', f'frame_{k}.jpg']), imgs[i])
                with open(os.sep.join([output, f'{folder}',f'CAM_{i+1}', f'CAM_{i+1}.txt']), 'a+t') as f:
